...
<?php
	...
	if(!empty($_GET['q'])){
		...
		$query = "SELECT photo_id, timestamp FROM Animal where person_id = $q AND species = 97";
		...
		if(mysqli_num_rows($result) == 0){
			...
			echo "<tr><th>PhotoID</th><th>Photo Uploaded Date</th><th>View</th></tr>";
			while($row = mysqli_fetch_assoc($result)){
				echo "<tr><td>{$row['photo_id']}</td><td>{$row['timestamp']}</td>";
				echo "<td><button class='editbtn' onclick = 'showpictureLiked(this)'>View</button></td>";
			}
			echo "</table>";
		}
	} elseif(!empty($_GET['i'])){
		...
		// I am not sure a filter should apply to this 'User Favourite Image' section or not, therefore 
		// leave as it is, but not display the filter in the actual page. So if in the future a filter
		// should apply to this section, it is easy to add it back by just showing the filter in the 
		// page.
...
function showpictureLiked(e){

	var hi = e.parentNode.parentNode.cells[0].innerHTML;
	$q = hi;
	$("#image3").load('showPicture.php?q='+$q);
}
</script>

			</div>
			<div class = "col-sm-6" id = "image3" style= "font-size:115%">Image Here</div>
		</div>
	</div>
</body>
</html>